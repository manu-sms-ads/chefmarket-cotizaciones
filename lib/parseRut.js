...
function extractNIT(text) {
  // Patrón principal: formulario DIAN (empieza en 14, 11-12 dígitos) + espacio + NIT+DV(10-11)
  // Los formularios DIAN siempre empiezan con "14" → 14XXXXXXXXX (11 dígitos) o 14XXXXXXXXXX (12)
...
function extractActividades(text) {
  ...
  // Fallback: actividad única (4 dígitos CIIU + fecha YYYYMMDD con año válido)
...
function extractFechas(text) {
  ...
  // Fallback actividad única: CIIU(4d) + fecha(YYYYMMDD con año válido) → un solo bloque de 12 dígitos
  const single12 = text.match(/\b\d{4}((?:19|20)\d{6})\b/);
  if (single12) {
    const fmt = (s) => `${s.slice(6, 8)}/${s.slice(4, 6)}/${s.slice(0, 4)}`;
    return { fechaInscripcion: fmt(single12[1]), fechaActualizacion: null };
  }
...
export function parseRutText(rawText) {
  ...
  // 1b. Persona jurídica: el nombre aparece justo ANTES del bloque COLOMBIA/169
  //     Buscamos directamente: [nombre legal]\n[líneas en blanco]COLOMBIA\n[169|código]
  //     Esto funciona independientemente de cuántos bloques COLOMBIA haya.
  if (!razonSocial) {
    const legalBeforeColombia = text.match(
      /\n([A-ZÁÉÍÓÚÑ][^\n]+?(?:S\.?\s*A\.?\s*S\.?|S\.?\s*A\.?|LTDA\.?|E\.?\s*A\.?\s*T\.?|S\.?\s*C\.?\s*S\.?|E\.?\s*S\.?\s*P\.|I\.?\s*P\.?\s*S\.|COOPERATIVA|FUNDACI[OÓ]N|CORPORACI[OÓ]N)[^\n]*)\n+COLOMBIA(?:\n|$)/i
    );
    if (legalBeforeColombia) {
      razonSocial = legalBeforeColombia[1].trim();
    }
  }
  ...
  if (departamento && lastDeptPos !== -1) {
    ...
    if (cityM) {
      municipio = cityM[1].trim();
    } else if (departamento === 'Bogotá D.C.' || departamento === 'Bogotá' || departamento === 'Distrito Capital') {
      // En el formulario DIAN, Bogotá D.C. es su propio municipio
  ...
  // CR = Carrera (abrev. corta), CL = Calle, KR/KRA = Carrera (DIAN), AV = Avenida, etc.
  const dirMatch = text.match(
    /\b(K|CR|CL|KR|KRA|AV|TV|DG|AC|CRA|CALLE|CARRERA|AVENIDA|DIAGONAL|TRANSVERSAL|VIA|MZ|LT)\b[ \t\d\w#\-\.]+/i
  );
  ...
  // Fijo colombiano: 10 dígitos empezando en 60x (601=Bogotá, 604=Medellín, etc.)
  // Fijo corto: 7 dígitos (sin indicativo de ciudad)
